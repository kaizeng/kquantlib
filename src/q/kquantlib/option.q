...
/[TODO]: refactor the price engine.
...
.ql.EuropeanOptionImpliedVolatility:{[option_type; option_value; spot_price; strike_price; dividend_rate; risk_free_rate; maturiy; volatility]
 today         : .z.D;
 maturity_date : .z.D + maturiy * 360;
 maturity_date : ql[`:Date][`dd$maturity_date; `mm$maturity_date; `year$maturity_date]; 
 option_type   : $[option_type = `call; ql[`:Option][`:Call]; option_type = `put; ql[`:Option][`:Put]; 'unsported_type];
 ...
 bsm_process: ql[`:BlackScholesMertonProcess][spot_handle;  dividend_yield;    flat_ts;  flat_vol_ts];
 european_option[`:setPricingEngine] ql[`:AnalyticEuropeanEngine][bsm_process];
 :european_option[`:impliedVolatility][option_value;bsm_process;1e-6;100]`
 }

/
.ql.EuropeanOption[`call; 100; 120; 0.0; 0.0; 1f; 0.2; ()!()]

.ql.EuropeanOptionImpliedVolatility[`call; 2.107961; 100; 120; 0.0; 0.0; 1f; .25]
